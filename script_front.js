...
// Flag para evitar múltiples popups
...
// Función para iniciar captura de pantalla con selector de área
function iniciarCapturaPantalla() {
    ...
    overlay.addEventListener('mouseup', async (e) => {
        ...
        // Mostrar loading
        mostrarRespuesta('Capturando...', true);
        
        // Capturar pantalla usando background script
        console.log('Enviando mensaje al background...');
        ...
        chrome.runtime.sendMessage({ action: 'captureScreen' }, async (response) => {
            ...
            if (response && response.error) {
                console.error('Error del background:', response.error);
            ...
            if (response && response.imageData) {
                try {
                    ...
                    // Recortar la imagen al área seleccionada
                ...
                mostrarRespuesta('Sin datos de captura', true);
            }
        });
    });
...
function mostrarRespuesta(respuesta, found = true, preguntaOriginal = null) {
  ...
  if (!found && preguntaOriginal) {
    ...
    // Botón de captura de pantalla (cámara)
  ...
  box.addEventListener("click", (e) => {
    // No hacer toggle si se clickeó un botón

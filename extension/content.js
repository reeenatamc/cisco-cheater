...
function mostrarRespuesta(respuesta, found = true, preguntaOriginal = null) {
  ...
  // Si respuesta es un objeto con estructura de BD (type, data, etc.)
  if (found && typeof respuesta === 'object' && respuesta !== null && !Array.isArray(respuesta)) {
    const { type, data, answers } = respuesta;
    
    if (type === 'SINGLE') {
      box.innerHTML = `<strong>✓</strong> ${data}`;
    } else if (type === 'MULTI') {
      if (Array.isArray(data)) {
        const items = data.map(item => `<div style="margin:2px 0">✓ ${item}</div>`).join('');
        box.innerHTML = items;
      } else {
        box.innerHTML = `<strong>✓</strong> ${data}`;
      }
      ...
      if (Array.isArray(data)) {
        const pairs = data.map(pair => `<div style="margin:2px 0">${pair.col_a} ➜ ${pair.col_b}</div>`).join('');
        box.innerHTML = pairs;
      } else {
        box.innerHTML = String(data);
      }
    } else {
      // Fallback para formato desconocido
      box.innerText = typeof data === 'string' ? data : JSON.stringify(data);
    }
    ...
    box.innerText = String(respuesta);
  } else {
      ...
      chrome.storage.local.get(['isActivated'], async function(result) {
          ...
          document.addEventListener("mouseup", async (e) => {
              ...
              if (seleccion.length > 0) {
                  try {
                      // Obtener deviceId cada vez para asegurar consistencia
